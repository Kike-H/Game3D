...
gd.core = {
	...
	// Ancho y alto del area de juego.
	...
	iniciar: function(width, height, ejecutar) {
		...
		if (!this.canvas.getContext) return alert('Por favor descargue' +
			'un navegador que soporte Canvas como Google Chrome para proceder.');
		gd.gl = this.canvas.getContext("experimental-webgl");

		if (gd.gl === null || gd.gl === undefined)
			return alert('Ey, su navegador no soporta WebGl. ' +
	...
	animar: function() {
		requestAnimFrame(gd.core.animar);
		gd.core.dibujar();
	},
	...
	sombreado: {
		...
		get: function(id) {
			...
			if (!gd.gl.getShaderParameter(this.sombreado, gd.gl.COMPILE_STATUS)) {
				alert('Error de compilacion del Shader: ' + gd.gl.getShaderInfoLog(this.sombreado));
				return null;
			}
